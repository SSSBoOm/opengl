...
{
  IDLE = 1,
  IDLE_WALK, // Idle to Walk
  WALK_IDLE, // Walk to Idle
  WALK,
  WALK_RUN, // Walk to Run
  RUN_WALK, // Run to Walk
  RUN,
  RUN_IDLE,      // Run to Idle
  IDLE_JUMPIDLE, // Idle to JumpIdle
  JUMPIDLE_IDLE, // JumpIdle to Idle
};
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    case IDLE_JUMPIDLE:
    {
      ...
      if (smoothBlend >= 1.0f)
      {
        blendAmount = 0.0f;
        float startTime = animator.m_CurrentTime2;
        animator.PlayAnimation(&idleJumpAnimation, NULL, startTime, 0.0f, blendAmount);
